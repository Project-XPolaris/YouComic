...
export const downloadFile = async (url,fileName,product) => {
  ...
  const request = await nodeHttp.get(url, (response) =>  {
    response.pipe(writer);
    const totalLength : any = response.headers['content-length'];
    let downloadSize = 0;
    let lastProgress = 0;
    response.on('data', (chunk) => {
      downloadSize += chunk.length;
      const progress = Number(((downloadSize / totalLength) * 100).toFixed());
